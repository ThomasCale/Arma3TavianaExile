...
UPDATEEMAIL='nicholascleal@gmail.com';
INFISTARVERSION='10-04-2020 18-44-01 - v0094';
...
diag_log format['<infiSTAR.de> %1 - VERSION: 10-04-2020 18-44-01 - v0094',time];
{
	_x params ['_uids'];
	{
		private _uid = _x;
...
if!(isClass (missionconfigfile >> 'RscListBox_infiSTAR'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_defines.hpp is not in your MPMission or you forgot to #include it within your description.ext   [10613817696x2 - 10-04-2020 18-44-01 - v0094 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'infiSTAR_AdminMenu'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [10613817696x2 - 10-04-2020 18-44-01 - v0094 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'infiSTAR_CHAT'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_chat.hpp is not in your MPMission or you forgot to #include it within your description.ext   [10613817696x2 - 10-04-2020 18-44-01 - v0094 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'RscListBox_infiSTAR'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_KeyBinds.hpp is not in your MPMission or you forgot to #include it within your description.ext   [10613817696x2 - 10-04-2020 18-44-01 - v0094 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(getNumber(missionconfigfile >> 'infiSTAR_AdminMenu' >> 'version') isEqualTo 3)exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp in your MPMission is outdated. Update it or infiSTAR will not load.   [10613817696x2 - 10-04-2020 18-44-01 - v0094 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
...
if(!("https://*" in _allowedHTMLLoadURIs) || !("http://*" in _allowedHTMLLoadURIs))exitWith{
	_log = format["<infiSTAR.de> %1 - infiSTAR_defines.hpp in your MPMission is outdated. Update it or infiSTAR will not load.   [10613817696x2 - 10-04-2020 18-44-01 - v0094 - %2 - %3]",time,serverName,productVersion];
	diag_log _log;
	["MPMission_ERROR",_log] call FNC_A3_CUSTOMLOG;
};
...
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!   [10613817696x2 - 10-04-2020 18-44-01 - v0094 - %2 - %3]',time,serverName,productVersion];
...
_fnc_RandomGen = {
	...
	_fnc_actualGen = {
		...
		_main	= ['a','e','7','7','a','2','6','3','8','1','6','c','7','3','1','7','7','1','8','0','8','3','9','3','7','c','1','5','5','3','6'];
...
_m = missionNamespace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=vi5306908848';missionNamespace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
...
_bigInputArray pushBack [
	[_name,_uid,_AHKickLog,_AHKickOFF,_admins,_admin,_isNormal,_server_classes],
	{
		...
		_devLog = format['infiSTAR.de AHAT %1 - 10-04-2020 18-44-01 - v0094 - server running: %2:10613817696x2',VERSIONCHECKRESULT,time call FN_GET_TIME_TIME];diag_log _devLog;
		"; if(!_HIDE_FROM_PLAYERS)then{ _asd = _asd + "
...
"; }; _asd = _asd + "
	...
	_log = format['%1(%2) | %3 | %4 [10-04-2020 18-44-01 - v0094]',_name,_uid,_log,time call FN_GET_TIME_TIME];
...
fn_infiSTAR_checkGlobalBanState = compileFinal ([_infiSTAR_checkGlobalBanState] call fnc_CompilableString);
